...
import json
import os
from user import User, load_users, save_users, is_valid_password, validate_input, sign_up, log_in
...
class TestUserModule(unittest.TestCase):
    ...
    def test_save_users(self, mock_json_dump, mock_file):
        users = {'test_user': {'username': 'test_user', 'password': 'hashed_pw', 'first_name': 'Test', 'last_name': 'User'}}
        save_users(users)
    ...
    def test_sign_up(self, mock_print, mock_prompt, mock_save_users, mock_load_users):
        username = sign_up()
        self.assertEqual(username, 'new_user')
        mock_print.assert_any_call("[bold green]Sign up successful![/bold green]")
